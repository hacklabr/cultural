...
class html {
    ...
    static function getImageUrl($filename, $complement = null) {
        ...
        foreach ($filenames as $fname) {
            if (file_exists(get_stylesheet_directory() . '/' . $fname)) {
                return get_stylesheet_directory_uri() . '/' . $fname;
            }

            if (file_exists(get_template_directory() . '/' . $fname)) {
    ...
    static function part($slug) {
        ...
        echo self::getPart($slug, $name, $_part_vars);
    }
    ...
    static function getVideo($video_url, $width = '100%', $height = '100%') {
        ...
        if ($video_type == 'vimeo') {
            ...
            $video_id = utils::getYoutubeVideoIdFromVideoURL($video_url);
            return "<iframe width='$width' height='$height' src='http://www.youtube.com/embed/$video_id' frameborder='0' allowfullscreen></iframe>";
        } else {
            return null;
        }
    }
    ...
    /**
     * retorna a url da imagem redimensionada e se esta não existir, cria esta imagem
     * @param type $filename o caminho para a imagem que será 
     * @param mixed $size as dimensões da nova imagem - pode ser um inteiro ou uma string com dois números separados por vírgula, ex: "300,200"
     * @param type $fit ver http://wideimage.sourceforge.net/examples/resizing/ - para 
     */
    static function getResizedImageURL($filename, $size, $fit = 'inside', $quality = 80) {
        $thumb_fname = utils::getResizedImagePath($filename, $size, $fit, $quality);
        if ($thumb_fname) {
            return str_replace(ABSPATH, home_url() . '/', $thumb_fname);
        } else {
            return '';
        }
    }
    
    static function resizedImageURL($filename, $size, $fit = 'inside', $quality = 80){
        echo self::getResizedImageURL($filename, $size, $fit, $quality);
    }
